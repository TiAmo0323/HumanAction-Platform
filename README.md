...
本文档根据 2026-07-23 的 `main` 分支代码整理；接口、默认参数和限制均以当前实现为准。
...
- 前端尚未统一 InterGen 与 LODGE 的重定向字段：LODGE 可直接优先播放重定向结果，InterGen 前端当前仍展示普通预览。
...
│  ├─ start_intergen_api_cpu.bat        # CPU 启动脚本
...
│  ├─ start_lodge_api_cpu.bat           # CPU 启动脚本
...
因此新任务的普通预览和重定向视频会使用同一份、最多 2000 帧的动作；实际视频时长取决于动作真实帧数。
...
前端提交后每 5 秒轮询任务状态；当 `retarget_status=succeeded` 且存在 `output_retarget_mp4_path` 时，优先播放重定向视频，否则播放普通预览。
...
如果依赖 `start_lodge_api_retarget.bat` 中的重定向环境变量，应先在同一终端设置这些变量，或同步调整启动脚本。
